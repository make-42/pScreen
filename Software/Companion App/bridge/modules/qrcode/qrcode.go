...
var QRCodeModule modules.Module = modules.Module{RenderFunction: func(im *image.RGBA) *image.RGBA {
	...
	dc.Clear()
	dc.SetRGB(0, 0, 0)
	qrcodeObj, err := qrcode.New(config.QRCodeContent, qrcode.Highest)
	utils.CheckError(err)
	qrcodeImg := qrcodeObj.Image(-1)
	qrcodeImgBounds := qrcodeImg.Bounds()
	offset := image.Pt(4, config.CanvasRenderDimensions.Y/2-qrcodeImgBounds.Dy()/2)
	...
	qrcodeGifOffset := image.Pt(qrcodeImgBounds.Dx()+8, config.CanvasRenderDimensions.Y-qrcodeGifBounds.Dy())
	...
	dc.DrawStringWrapped(config.QRCodeTitle, float64(qrcodeImgBounds.Dy())+8, 4, 0, 0, float64(config.CanvasRenderDimensions.X)-float64(qrcodeImgBounds.Dy())-12, 1, gg.AlignRight)
	dc.SetFontFace(renderer.TinyFont)
	dc.DrawStringWrapped(config.QRCodeDescription, float64(qrcodeImgBounds.Dy())+8, 32, 0, 0, float64(config.CanvasRenderDimensions.X)-float64(qrcodeImgBounds.Dy())-12, 1, gg.AlignRight)
	CurrentGIFFrame++

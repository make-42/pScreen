...
func GetMediaProgressBarYFromX(x float64) float64 {
	return float64(config.CanvasRenderDimensions.Y-2) - float64(config.MediaProgressBarIndicatorRadius) + (math.Sin(float64(x)*math.Pi*config.MediaProgressBarWaveScale)-1)*(float64(config.MediaProgressBarHeight)/2-2)
}

func DrawMediaProgressBar(dc *gg.Context, positionSec, durationSec float64) {
	// This function does not handle the font
	dc.DrawStringAnchored(utils.FormatDuration(positionSec), 4, float64(config.CanvasRenderDimensions.Y-7)-float64(config.MediaProgressBarHeight), 0, 0)
	dc.DrawStringAnchored(utils.FormatDuration(durationSec), float64(config.CanvasRenderDimensions.X-4), float64(config.CanvasRenderDimensions.Y-7)-float64(config.MediaProgressBarHeight), 1, 0)

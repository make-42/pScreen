...
module panel()
{
    ...
    intersection()
    {
        translate(asanoha_anchor) drawAsanoha();
        asanoha_area(asanoha_anchor);
    }
}
...
module screen_screw_set()
{
    ...
    translate([ screen_pcb_screw_holes_spacing.x + screen_pcb_size.x * (screen_count - 1), 0, 0 ])
    ...
    translate([ 0, screen_pcb_screw_holes_spacing.y, 0 ])
        nut_hole(thread_depth = panel_thickness - m2_screw_nut_depth, nut_depth = m2_screw_nut_depth,
                 nut_diameter = m2_screw_nut_diameter, thread_diameter = m2_screw_diameter);
    translate([
        screen_pcb_screw_holes_spacing.x + screen_pcb_size.x * (screen_count - 1), screen_pcb_screw_holes_spacing.y, 0
    ]) nut_hole(thread_depth = panel_thickness - m2_screw_nut_depth, nut_depth = m2_screw_nut_depth,
                nut_diameter = m2_screw_nut_diameter, thread_diameter = m2_screw_diameter);
}
...
{
    rpip_anchor = [ pi_board_offset.x, pi_board_offset.y + screen_anchor_coords.y, 0 ];
...
module screens()
{
    ...
    screw_anchor_coords = [
        screen_anchor_coords.x + screen_pcb_screw_holes_center_from_board_edge_distance.x,
        screen_anchor_coords.y - screw_dist, 0
    ];
...
module stand_leg(invert = false)
{
    ...
    translate([0,panel_size.y-stand_size.y,0])
    difference()
    {
        union()
        {
